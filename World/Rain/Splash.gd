...
export var splash_area = 15
export var splashes_per_second = 300
export var splash_pool_size = 200
var splash_obj = preload("res://World/Rain/Splash.tscn")
var splashes = []
...
func make_splash():
	...
	var start_pos = global_transform.origin + Vector3(x_pos, 30, z_pos)
	var space_state = get_world().direct_space_state
	var result = space_state.intersect_ray(start_pos, start_pos - Vector3(0, 400, 0))
	if result.size() > 0:
		splashes[cur_splash_ind].global_transform.origin = result.position
